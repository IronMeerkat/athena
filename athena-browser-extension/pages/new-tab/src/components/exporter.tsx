...
export function MessageExporter() {
  ...
  const selectedMessages = useMemo(() => {
    ...
    // Context export disabled (masks removed)
...
export function PreviewActions(props: {
  ...
  const onRenderMsgs = (msgs: ChatMessage[]) => {
    ...
    // Provider sharing disabled in Athena mode
...
export function ImagePreviewer(props: {
  ...
  const chatStore = useChatStore();
  const session = chatStore.currentSession();
  const mask = { modelConfig: { model: "athena-default" }, avatar: "2699-fe0f" } as any;
  const config = useAppConfig();
    ...
    <div className={styles["image-previewer"]}>
      ...
      >
        <div className={styles["chat-info"]}>
          ...
          <div>
            <div className={styles["main-title"]}>Athena</div>
            <div className={styles["sub-title"]}>github.com/Athena/athena</div>
