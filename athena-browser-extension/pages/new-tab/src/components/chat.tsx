// Local fallback debounce to avoid dependency
const useDebouncedCallback = (fn: any, delay: number) => {
  let timer: any;
  return (...args: any[]) => {
    clearTimeout(timer);
...
// Removed next/dynamic, import Markdown directly

// Controllers retained for local stop/retry only
...
type MultimodalContent = any;
import { createTTSPlayer } from "../utils/audio";
import { MsEdgeTTS, OUTPUT_FORMAT } from "../utils/ms_edge_tts";

const isEmpty = (v: unknown) =>
  v == null || (Array.isArray(v) ? v.length === 0 : String(v).length === 0);
// Provider helpers retained only for grouping; Athena manages provider
import { getModelProvider } from "../utils/model";
import { RealtimeChat } from "./realtime-chat/realtime-chat";
...
// SessionConfigModel removed in Athena mode
...
export function ChatActions(props: {
  ...
  // switch model
  const currentModel = "athena-managed";
  const currentProviderName = ServiceProvider.Athena;
  ...
  const modelSizes = [] as any[];
  const dalle3Qualitys: DalleQuality[] = ["standard", "hd"];
  const dalle3Styles: DalleStyle[] = ["vivid", "natural"];
  const currentSize = "1024x1024" as ModelSize;
  const currentQuality = "standard" as DalleQuality;
  const currentStyle = "vivid" as DalleStyle;
  ...
  useEffect(() => {
    ...
    const isUnavailableModel = false;
    if (isUnavailableModel && models.length > 0) {
      // show next model to default model if exist
      // no-op in Athena mode
    }
      ...
      <>
        ...
        {ATHENA_MODE ? (
          <div style={{ opacity: 0.5, pointerEvents: "none" }}>
            <ChatAction
              onClick={() => {}}
              text={currentModelName || "Model (managed by Athena)"}
              icon={<RobotIcon />}
            />
          </div>
        ) : (
        ...
        {!ATHENA_MODE && showModelSelector && (
          <Selector
            ...
            onSelection={(s) => {
              ...
              // disabled in Athena mode
            }}
          />
        )}

        {false && supportsCustomSize(currentModel) && (
          <ChatAction
  ...
  return (
    <div className="modal-mask">
      ...
      >
        ...
        {/* Context prompts removed in Athena mode */}
      </Modal>
    </div>
  );
}
...
function _Chat() {
  ...
  async function openaiSpeech(text: string) {
    if (speechStatus) {
      ...
      if (config.ttsConfig.engine !== DEFAULT_TTS_ENGINE) {
        ...
        // Athena-managed TTS disabled; use Edge-TTS always
        audioBuffer = await new MsEdgeTTS().toArrayBuffer(textContent);
      }
  ...
  useCommand({
    ...
    code: (text) => {
      ...
      showConfirm(Locale.URLCommand.Code + `code = ${text}`).then((res: boolean) => {
        if (res) {
  ...
  const handlePaste = useCallback(
    async (event: React.ClipboardEvent<HTMLTextAreaElement>) => {
      ...
      const items = (event.clipboardData as any)?.items || [];
