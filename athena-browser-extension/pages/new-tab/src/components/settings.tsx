...
import { SubmitKey, useChatStore, Theme, useAccessStore, useAppConfig } from "../store";
...
// Removed next/link usage; use regular anchor
import {
...
// Router optional, avoid hard dependency during lint
...
export function Settings() {
  ...
  const [checkingUpdate] = useState(false);
  const currentVersion = "0.0.0";
  const remoteId = "0.0.0";
  const hasNewVersion = false;
  const updateUrl = getClientConfig()?.isApp ? RELEASE_URL : UPDATE_URL;
  function checkUpdate(_force = false) {}

  const accessStore = useAccessStore();
  const shouldHideBalanceQuery = useMemo(() => true, []);
  ...
  const enabledAccessControl = useMemo(() => false, []);
  ...
  const ATHENA_MODE = true;
  const openAIConfigComponent = !ATHENA_MODE && accessStore.provider ===
  ...
  const azureConfigComponent = !ATHENA_MODE && accessStore.provider ===
  ...
  const googleConfigComponent = !ATHENA_MODE && accessStore.provider ===
  ...
  const anthropicConfigComponent = !ATHENA_MODE && accessStore.provider ===
  ...
  const baiduConfigComponent = !ATHENA_MODE && accessStore.provider ===
  ...
  const tencentConfigComponent = !ATHENA_MODE && accessStore.provider ===
  ...
  const byteDanceConfigComponent = !ATHENA_MODE && accessStore.provider ===
  ...
  const alibabaConfigComponent = !ATHENA_MODE && accessStore.provider ===
  ...
  const moonshotConfigComponent = !ATHENA_MODE && accessStore.provider ===
  ...
  const deepseekConfigComponent = !ATHENA_MODE && accessStore.provider ===
  ...
  const XAIConfigComponent = !ATHENA_MODE && accessStore.provider === ServiceProvider.XAI && (
    <>
      <ListItem
  ...
  const chatglmConfigComponent = !ATHENA_MODE && accessStore.provider ===
  ...
  const siliconflowConfigComponent = !ATHENA_MODE && accessStore.provider ===
  ...
  const stabilityConfigComponent = false && accessStore.provider ===
  ...
  const lflytekConfigComponent = !ATHENA_MODE && accessStore.provider ===
  ...
  return (
    <ErrorBoundary>
      ...
      <div className={styles["settings"]}>
        ...
        <List>
          {/* Mask splash and builtin controls removed in Athena mode */}
        </List>
            ...
            <div style={{ opacity: 0.5, pointerEvents: 'none' }}>
              ...
              {accessStore.useCustomConfig && (
                <>
                  ...
                  {/* Provider-specific inputs disabled in Athena mode */}
