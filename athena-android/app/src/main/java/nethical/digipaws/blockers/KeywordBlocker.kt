...
class KeywordBlocker(val service: AccessibilityService) : BaseBlocker() {
    ...
    // keyword -> uptime end time
    private val tempWhitelistedKeywords: MutableMap<String, Long> = mutableMapOf()

    fun addTempWhitelist(keyword: String, endUptimeMillis: Long) {
        tempWhitelistedKeywords[keyword.lowercase(Locale.ROOT)] = endUptimeMillis
    }
    ...
    private fun containsBlockedKeyword(url: String): String? {
        ...
        keywords.forEach { word ->
            ...
            if (blockedKeyword.contains(lower)) {
                val end = tempWhitelistedKeywords[lower]
                if (end != null) {
                    if (SystemClock.uptimeMillis() < end) return@forEach
                    else tempWhitelistedKeywords.remove(lower)
                }
                return word
            }
        }
