...
class FocusModeBlocker : BaseBlocker() {
    ...
    fun doesAppNeedToBeBlocked(packageName: String): FocusModeResult {
        ...
        if (focusModeData.isTurnedOn) {
            // Whitelist/Blacklist overrides for manual mode
            if (focusModeData.whitelist.contains(packageName)) {
                return FocusModeResult(isBlocked = false)
            }
            if (focusModeData.blacklist.contains(packageName)) {
                return FocusModeResult(isBlocked = true, focusModeEndTime = focusModeData.endTime)
            }
        ...
        // check if app is under auto-focus mode (whitelist overrides)
        if (focusModeData.whitelist.contains(packageName)) {
            return FocusModeResult(isBlocked = false)
        }
        if (focusModeData.blacklist.contains(packageName)) {
            val endAutoFocusBlack = getEndTimeInMillis(packageName)
            return FocusModeResult(isBlocked = true, focusModeEndTime = endAutoFocusBlack ?: -1)
        }
        // Default auto-focus evaluation
    ...
    data class FocusModeData(
        ...
        var whitelist: HashSet<String> = hashSetOf(),
        var blacklist: HashSet<String> = hashSetOf()
    )
